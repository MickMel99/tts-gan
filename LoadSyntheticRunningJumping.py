...
class Synthetic_Dataset(Dataset):
    def __init__(self, 
                 ...
                 # '.\pre-trained-model\JumpingGAN_checkpoint',
        ...
        running_ckp = torch.load(Running_model_path, map_location='cpu')
        ...
        jumping_ckp = torch.load(Jumping_model_path, map_location='cpu')
        jumping_gen_net.load_state_dict(jumping_ckp['gen_state_dict'])
        
        
        #generate synthetic running data label is 0
        z = torch.Tensor(np.random.normal(0, 1, (self.sample_size, 100)))
